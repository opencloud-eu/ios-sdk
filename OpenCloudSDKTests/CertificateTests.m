...
- (void)testCertificateMetaDataParsing
{
	...
	OCCertificate *certificate = [self certificateNamed:@"fake-demo_opencloud_org" hostName:@"demo.opencloud.eu"];

	XCTAssert(((metaData = [certificate metaDataWithError:&parseError]) != nil), @"Metadata returned");
	XCTAssert([((NSDictionary *)metaData[OCCertificateMetadataSubjectKey])[OCCertificateMetadataCommonNameKey] isEqual:@"demo.opencloud.eu"], @"Common name is correct");
...
- (void)testCertificateMetaDataParsingFromWeb
{
	NSArray <NSURL *> *urlsToTest = @[
		[NSURL URLWithString:@"https://opencloud.eu/"],
...
- (void)testCertificateRuleChecker
{
	...
	OCCertificate *demoCertNew = [OCCertificate assembleChain:@[
		...
		[self certificateNamed:@"demo-cert-new" 	hostName:@"demo.opencloud.eu"]
	]];

	OCCertificate *demoCertOld = [OCCertificate assembleChain:@[
		[self certificateNamed:@"demo-cert-root" 	hostName:nil],
		[self certificateNamed:@"demo-cert-letsencrypt" hostName:nil],
		[self certificateNamed:@"demo-cert-old" 	hostName:@"demo.opencloud.eu"]
	]];
	...
	OCCertificate *letsEncryptCertOldRoot = [OCCertificate assembleChain:@[
		...
		[self certificateNamed:@"letsencrypt-old" hostName:@"demo.opencloud.eu"],
	]];
	...
	OCCertificate *letsEncryptCertNewRoot = [OCCertificate assembleChain:@[
		...
		[self certificateNamed:@"letsencrypt-new" hostName:@"demo.opencloud.eu"],
	]];
	...
	XCTAssert(RunCheck(demoCertNew, nil, @"certificate.commonName == \"opencloud.eu\"") == YES);
