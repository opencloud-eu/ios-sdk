...
- (void)testStartStopCoreAndEraseVault
{
	...
	// Create bookmark for demo.opencloud.eu
...
- (void)testDuplicateNameSuggestions
{
	...
	[core startWithCompletionHandler:^(OCCore *core, NSError *error) {
		...
		[core fetchUpdatesWithCompletionHandler:^(NSError * _Nullable error, BOOL didFindChanges) {
			...
			[core suggestUnusedNameBasedOn:@"OpenCloud Manual.pdf" atLocation:OCLocation.legacyRootLocation isDirectory:NO usingNameStyle:OCCoreDuplicateNameStyleCopy filteredBy:nil resultHandler:^(NSString * _Nullable suggestedName, NSArray<NSString *> * _Nullable rejectedAndTakenNames) {
				XCTAssert([suggestedName isEqual:@"OpenCloud Manual copy.pdf"]);
			...
			// - style: bracketed
			dispatch_group_enter(suggestionWaitGroups);
			[core suggestUnusedNameBasedOn:@"OpenCloud Manual.pdf" atLocation:OCLocation.legacyRootLocation isDirectory:NO usingNameStyle:OCCoreDuplicateNameStyleBracketed filteredBy:nil resultHandler:^(NSString * _Nullable suggestedName, NSArray<NSString *> * _Nullable rejectedAndTakenNames) {
				XCTAssert([suggestedName isEqual:@"OpenCloud Manual (1).pdf"]);
				XCTAssert(rejectedAndTakenNames.count == 1);
			...
			dispatch_group_enter(suggestionWaitGroups);
			[core suggestUnusedNameBasedOn:@"OpenCloud Manual.pdf" atLocation:OCLocation.legacyRootLocation isDirectory:NO usingNameStyle:OCCoreDuplicateNameStyleCopy filteredBy:^BOOL(NSString * _Nonnull suggestedName) {
				return ![suggestedName isEqual:@"OpenCloud Manual copy.pdf"];
			} resultHandler:^(NSString * _Nullable suggestedName, NSArray<NSString *> * _Nullable rejectedAndTakenNames) {
				XCTAssert([suggestedName isEqual:@"OpenCloud Manual copy 2.pdf"]);
